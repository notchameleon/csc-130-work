...
/*
 * 
 * Road class that manages vehicle positions & collisions.
 * 
 * PROJECT 2 Implements history tracking using LinkedStacks & crashed vehicle pool using LinkedQueue.
 * @author Steff ;)
 */
...
public class Road {
	...
	private LinkedStack<Vehicle[]> historyStack; // Undo functionality history tracking.
	 ...
	 * getSize() - int
	 * returns the 
	 *amount of positions in the road array.
	...
	/* placeVehicle() - boolean
	 * Places a vehicle at a specified position
	 * @param position the positon to place the vehicle 
	...
	/*
	 * 
	 * getVehicleAt() - Vehicle
	 * gets the vehicle at the specified position
	 * @param position the position to check 
	 * @return the vehicle at that position.
	 */
	...
	/*
	 * moveVehicle(int fromPosition, int toPosition) -  boolean
	 * 
	 ...
	 * Processes vehicle movements for the specified number of steps
	 *@param steps,  the number of steps of movement direction (-1 , 0 , 1)
	 */

	public void processMovements(int steps)
	{
		...
		for (int step = 0; step < stepCount; step++)
		{
			...
			System.out.println("\n---> Movement step " + (step + 1) + " of " + stepCount + " (" + getDirectionName(direction) + ")");


			int[] desiredPositions = new int[road.length];
			...
			if (direction == 1) 
			{
				for (int i = road.length - 1; i >= 0; i--)
				{
					if (road[i] != null && road[i].isActive())
					{
						...
						int newPos = i + direction;
						if ( newPos >= 0 && newPos < road.length) 
						{
							desiredPositions[i] = newPos;
						}
					}
				}	
			} 
			...
			for (int i = 0; i < road.length; i++) 
			{
				if (desiredPositions[i] != -1)
				{
					executeSingleMove(i, desiredPositions[i], direction);
				}
			}
	...
	/*
	 * executeSingleMove() - void
	 * Executes a single vehicle movement & handle collisions
	 * @param currrentPos current position of vehicle
	 * @param newPos desired new position
	 * @param direction direction of movement
	 */

	private void executeSingleMove(int currentPos, int newPos , int direction)
	{
		Vehicle movingVehicle = road[currentPos];
		...
		if (newPos < 0 || newPos >= road.length)
		{
			System.out.println(movingVehicle + " cannot move " + getDirectionName(direction) 
			+ " from " + currentPos + " (out of bounds)");
			return;
		}

		Vehicle targetVehicle = road[newPos];
		System.out.print(movingVehicle + " attempting to move from " + currentPos + " to " + newPos);
		...
		if (targetVehicle != null && targetVehicle.isActive())
		{
			...
			System.out.println("----> SUCCESS MATE");

			road[newPos] = movingVehicle;
			road[currentPos] = null;
		}
	}
	 ...
	 * @return String representatino of direction.
	 */
	...
	/*
	 *handleCollision(Vehicle moving, Vehicle target, int oldPos, int newPos)
	 *- void.
	 *
	 * Handle collision between two vehicle
	 * @param moving the moving vehicle
	 * @param target the target vehicle being hit
	 * @param oldPos oold position of moving vehicle
	 * @param newPos new position *collision point*
	 * 
	 */
	private void handleCollision(Vehicle moving, Vehicle target, int oldPos, int newPos)
	{
		...
		if (moving instanceof Car && target instanceof Bus) 
		{
			...
			System.out.println("  ROAD STATE AFTER COLLISION:");
			displayRoad();
			System.out.println("");
		...
		else if (moving instanceof Car && target instanceof Car) 
		{
			...
			if (!car1.getColor().equals(car2.getColor())) 
			{
				...
				addToCrashedPool(car1);
				addToCrashedPool(car2);
				...
				if (random.nextBoolean()) { // 50% chance to add new car
					addRandomCar();
					System.out.println("  ROAD STATE AFTER ADDING NEW CAR:");
					displayRoad();
					System.out.println("");
	...
	/*
	 * 
	 * findEmptyPosition() - int
	 * Helper method to find empty position 
	 * @return index of first empty position
	 */
	...
	private void addRandomCar() 
	{
		...
		Vehicle reusedVehicle = getFromCrashPool();
		if (reusedVehicle != null && reusedVehicle instanceof Car) {
			int emptyPos = findEmptyPosition();
			if (emptyPos != -1) {
				reusedVehicle.setActive(true);
				road[emptyPos] = reusedVehicle; // ✅ Use the reused vehicle!
				return;
			}
		}
	 ...
	 * uses the linkstacked push to get a current road state 
	 * and saves it in the history of stack.
	 * 
	 */
	...
	/*
	 * processCommandsFromFile(Road road, String fileName) - void
	 * @param road, the road to operate on
	 * @param fileName - the name of the input
	 * @throws FileNotFoundException if file name isnt found 
	 */

	public static void processCommandsFromFile(Road road, String fileName) throws FileNotFoundException {
		...
		try {
			...
			System.out.println("\n--- INPUT FILE CONTENT ---");
			Scanner displayScanner = new Scanner(new File(fileName));
			...
			{			
				...
				if (!scnr.hasNextInt())
				{
					System.out.println("Error: Missing number after " + operation);
					break;
				}
				...
				System.out.println("\n--- STEP " + step + " ---");
				System.out.println("Command: " + operation + " " + value);

				System.out.println("INITIAL ROAD STATE:");
				road.displayRoad();

				if (operation.equals("m")) {
					System.out.println("ACTION: Move " + value + " space(s) "
							+ (value > 0 ? "Forward" : value < 0 ? "BACKWARD" : "STAY"));
					road.processMovements(value);
				} else if (operation.equals("u")) {
					System.out.println("ACTION: Undo " + value + " step(s)");
					road.undo(value);
				} else {
					System.out.println("ERROR: Unknown command '" + operation + "'");
				}

				System.out.println("FINAL ROAD STATE AFTER STEP " + step + ":");
